...
def _curated_study_record() -> dict[str, object]:
    return {
        ...
        "platforms": [{"accession": "GPL999", "name": "Temporary platform"}],
        "sample_count": 4,
        "case_sample_count": 2,
        ...
        "publication_urls": [],
        "curation_status": "temporary test record",
        "pipeline_active": True,
        "expression_candidate": False,
        "phenotype_label_status": "verified from test fixture",
...
def test_bronze_ingestion_writes_snapshots_and_manifest(tmp_path: Path) -> None:
    ...
    assert [study["study_accession"] for study in studies] == [
        "GSE224615",
        "GSE270045",
        "GSE267625",
    ]
    assert studies[0]["curation_status"] == "primary metadata candidate"
    assert studies[1]["expression_candidate"] is True
    assert studies[2]["phenotype_label_status"] == "needs verification"
    assert "metadata_only" in studies[2]["metadata_quality_flags"]
    assert "ingested_at" in studies[0]
    assert saved_manifest["stage"] == "bronze_ingestion"
    assert saved_manifest["record_count"] == 3
    assert "Read 8 curated registry record(s)." in manifest.notes
    assert "Skipped 5 review-only record(s)." in manifest.notes
...
def test_bronze_ingestion_uses_project_config_when_cwd_changes(
    ...
    assert manifest.record_count == 3
    assert studies[0]["study_accession"] == "GSE224615"


def test_bronze_ingestion_skips_review_only_records(tmp_path: Path) -> None:
    active_record = _curated_study_record()
    review_record = {
        **_curated_study_record(),
        "study_accession": "GSE888888",
        "pipeline_active": False,
        "curation_status": "review candidate",
        "phenotype_label_status": "needs review",
    }
    config_path = tmp_path / "curated_studies.json"
    config_path.write_text(json.dumps([active_record, review_record]), encoding="utf-8")
    ...
    studies = json.loads(
        (tmp_path / "storage" / "bronze" / "studies.json").read_text(
            encoding="utf-8"
        )
    )

    assert manifest.record_count == 1
    assert [study["study_accession"] for study in studies] == ["GSE999999"]
    assert "Read 2 curated registry record(s)." in manifest.notes
    assert "Skipped 1 review-only record(s)." in manifest.notes


def test_bronze_study_snapshot_rejects_inconsistent_sample_counts() -> None:
    record = _curated_study_record()
    record["platform"] = "GPL999 Temporary platform"
