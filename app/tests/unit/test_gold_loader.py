...
from app.ui.data_access.gold_loader import GoldArtifactNotFoundError, load_gold_bundle
...
def test_load_gold_bundle_returns_validated_bundle(tmp_path: Path) -> None:
    bundle_path = tmp_path / "gold_artifact_bundle.json"
    bundle_path.write_text(json.dumps(_bundle().model_dump(mode="json")), encoding="utf-8")

    bundle = load_gold_bundle(bundle_path)
...
def test_load_gold_bundle_raises_clear_error_for_missing_file(tmp_path: Path) -> None:
    missing_path = tmp_path / "missing_gold_bundle.json"

    with pytest.raises(GoldArtifactNotFoundError, match="Gold artifact bundle not found"):
        load_gold_bundle(missing_path)
